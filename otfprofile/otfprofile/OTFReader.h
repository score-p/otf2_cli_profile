...
private:
     ...
     */
    static int
    handle_def_creator( void* userData,
                        uint32_t stream,
                        const char* creator,
     ...
     */
    static int
    handle_def_version( void* userData,
                        uint32_t stream,
                        uint8_t major,
                        uint8_t minor,
                        uint8_t sub,
     ...
     */
    static int
    handle_def_comment( void*       userData,
                        uint32_t    stream,
                        const char* comment,
                        OTF_KeyValueList *list);
     ...
     */
    static int
    handle_def_timerres( void*    userData,
                         uint32_t stream,
     ...
     *  @param parent       Previously declared parent process identifier or 0 if process has no parent.
     *
     *  @param list         Pointer to an OTF_KeyValueList() that contains individual data.
     *
     *  @return             OTF_RETURN_ABORT  for aborting the reading process immediately
     ...
     */
    static int
    handle_def_process( void*       userData,
                        uint32_t    stream,
                        uint32_t    process,
                        const char* name,
                        uint32_t    parent,
     ...
     *  @param procs            Vector of process identifiers as provided by OTF_Handler_DefProcess().
     *
     *  @param list             Pointer to an OTF_KeyValueList() that contains individual data.
     *
     *  @return                 OTF_RETURN_ABORT  for aborting the reading process immediately
     ...
     */
    static int
    handle_def_processgroup( void*           userData,
                             uint32_t        stream,
                             uint32_t        procGroup,
                             const char*     name,
     ...
     */
    static int
    handle_def_functiongroup( void*       userData,
                              uint32_t    stream,
                              uint32_t    funcGroup,
                              const char* name,
                              OTF_KeyValueList *list);
     ...
     *  @param funcGroup            A function group identifier preliminary provided by OTF_Handler_DefFunctionGroup()
     *                              or 0 for no function group assignment.
     *  @param source               Reference to the function's source code location preliminary provided by
     *                              OTF_Handler_DefScl() or 0 for no source code location assignment.
     *
     *  @param list                 Pointer to an OTF_KeyValueList() that contains individual data.
     *
     *  @return                     OTF_RETURN_ABORT  for aborting the reading process immediately
     ...
     */
    static int
    handle_def_function( void*       userData,
                         uint32_t    stream,
                         uint32_t    func,
                         const char* name,
                         uint32_t    funcGroup,
     ...
     *  @param type                 One of the five supported collective classes: OTF_COLLECTIVE_TYPE_UNKNOWN (default),
     ...
     */
    static int
    handle_def_collop( void*       userData,
                       uint32_t    stream,
                       uint32_t    collOp,
                       const char* name,
                       uint32_t    type,
     ...
     *  @param unit                 Unit of the counter e.g. "#" for "number of..." or 0 for no unit.
     *
     *  @param list                 Pointer to an OTF_KeyValueList() that contains individual data.
     *
     *  @return                     OTF_RETURN_ABORT  for aborting the reading process immediately
     ...
     */
    static int
    handle_def_counter( void*       userData,
                        uint32_t    stream,
                        uint32_t    counter,
                        const char* name,
                        uint32_t    properties,
     ...
     */
    static int
    handle_def_keyvalue( void* userData,
                         uint32_t stream,
                         uint32_t key,
                         OTF_Type type,
                         const char *name,
     ...
     *  @param group                Process-group to which sender and receiver belong to or 0 for no group assignment.
     *  @param type                 Message type information > 0 or 0 for no information.
     *  @param length               Optional message length information.
     *  @param source               Explicit source code location identifier > 0 or 0 if no source information available.
     *
     *  @param list                 Pointer to an OTF_KeyValueList() that contains individual data.
     *
     *  @return                     OTF_RETURN_ABORT  for aborting the reading process immediately
     ...
     *  @param time                 The time when the message was send.
     *  @param process              Process where counter measurment took place.
     *  @param collOp               Collective identifier as defined with OTF_Handler_DefCollectiveOperation().
     ...
     *  @param matchingId           Identifier for finding the associated end collective event record.
     ...
     */
    static int
    handle_begin_collop( void*    userData,
                         uint64_t time,
                         uint32_t process,
                         uint32_t collOp,
                         uint64_t matchingId,
     ...
     */
    static int
    handle_end_collop( void*    userData,
                       uint64_t time,
                       uint32_t process,
                       uint64_t matchingId,
                       OTF_KeyValueList *list);
     ...
     */
    static int
    handle_function_summary( void* userData,
                             uint64_t time,
                             uint32_t function,
                             uint32_t process,
     ...
     */
    static int
    handle_message_summary( void* userData,
                            uint64_t time,
                            uint32_t process,
                            uint32_t peer,
                            uint32_t comm,
     ...
     */
    static int
    handle_collop_summary( void *userData,
                           uint64_t time,
                           uint32_t process,
                           uint32_t comm,
                           uint32_t collective,
     ...
     *  @param userData             Pointer to user data.
     *  @param time                 Time when summary was computed.
     *  @param process              If 'time' equals (uin64_t) -1, the unknown record is a definiton record
     *                              and 'process' represents the streamid of the record. If 'time' has a
     *                              valid value ( not (uint64)-1 ) the unknown record is an event-, statistics-
     *                              or snapshotrecord and 'process' represents the processid of the record.
     ...
     */
    static int
    handleUnknownRecord( void*       userData,
                         uint64_t    time,
                         uint32_t    process,
                         const char* record );
     ...
     *  @param communicator         Communicator identifier to match put start with it's end (together with tag)
     *  @tag                        Tag to match put start with it's end (together with communicator).
     ...
     *  @param origin               Initiating process / thread
     *  @param target               Targetedd process / thread
     *  @param communicator         Communicator identifier to match get start with it's end (together with tag)
     *  @tag                        Tag to match get start with it's end (together with communicator).
