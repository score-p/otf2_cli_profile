...
/*
 * Functor to sort based on exclusive time.
 */

struct ExclTimeExtract_t{
   ExclTimeExtract_t() {}
   typedef double val_type;

   val_type
   operator()(const itFunc_t& it){
      return it->second.excl_time.sum;
   }
};

/*
 * Functor to sort based on number of invocations.
 */ 
struct InvocExtract_t{
   InvocExtract_t() {}
   typedef uint64_t val_type;

   val_type
   operator()(const itFunc_t& it){
      return it->second.count.sum;
   }
};

/*
 * Sort the global data.
 * The order of the sort is based on the Functor T,
 * used to extract the key from the iterator and to
 * determine the type.
 * @param sortedMap The map to hold the ordered data
 * @param alldata   Global data to sort
 */ 
template<class T>
static void sort_data(std::multimap<typename T::val_type, itFunc_t>& sortedMap, const struct AllData& alldata)
{
    ...
    while (itend != it) {
       ...
       if (it->second.count.cnt) {
          sortedMap.insert(pair<typename T::val_type, itFunc_t> (
                   (typename T::val_type) (extract(it)), it));
       }
       it++;
    }
...
/*
 * Write the body of the table function. 
 * The template parameter T determines the order of the elements
 * in the table. 
 * @param tex     the given file stream (reference) 
 * @param alldata the global data
 * @param max     maximum number of entries in the table
 */ 
template<class T> 
static void write_functionTableBody(fstream& tex, struct AllData& alldata, const int max)
{
    int count = 0;

    typename std::multimap<typename T::val_type, itFunc_t> sortedMap;
    /* sort based on given functor into multimap */
    sort_data<T>(sortedMap, alldata);

    typename std::multimap<typename T::val_type, itFunc_t>::const_reverse_iterator it =
            sortedMap.rbegin();
    typename std::multimap<typename T::val_type, itFunc_t>::const_reverse_iterator itend =
            sortedMap.rend();
 ...
 * The order of the entries is based on the accumulated
 * exclusive time.
 *
 * @param tex the given file stream (reference)
 * @param alldata the global data
 */
static void write_functionTableExTime(fstream& tex, struct AllData& alldata) {
    ...
    write_functionTableBody<ExclTimeExtract_t>(tex, alldata, max);
...
/*
 * Write a latex function summary table to the given file stream.
 * The order of the entries is based on the number of Invocations.
 * @param tex the given file stream (reference)
 * @param alldata the global data
 */
static void write_functionTableInvoc(fstream& tex, struct AllData& alldata) {
    int mapSize = alldata.functionMapGlobal.size();
    int max = FUNC_TABLE_LEN;
    ...
    tex << "   \\hline" << endl;
    tex << "   \\bf Function & \\bf invocations[\\#] $\\nabla$ & "
            << "\\bf excl. time[sec] & \\bf incl. time[sec] \\\\"
            << endl;
    tex << "   \\hline\\hline" << endl;

    /* write the function table */
    write_functionTableBody<InvocExtract_t>(tex, alldata, max);
...
static void write_Dispersion(fstream& tex, struct AllData& alldata)
{
  ...
  while ( itend != it ) {
    ...
    /* min, 1/4 quartile, 1/4pos, median, medianpos, 3/4 quartile, 3/4pos, max */
    {
      tex << "\\boxplotlh{";
      tex.precision(7);
      tex.setf(ios::dec, ios::scientific);
...
static void write_Dispersion_histogram(fstream& tex, struct AllData& alldata) {
    ...
    while (it != itend) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            os << "    \\draw (0.4," << (i * ((double) (BP_HEIGHT - 1) / 5))
                ...
                if( i == 0 ) {
                        ...
                        os << ") node[below left]{$" << (i * ((BP_HEIGHT - 1) / 5))
                        * (sum_max / 5) << "$} -- (0.5," << (i
                        * ((double) (BP_HEIGHT - 1) / 5)) + 0.5 << ");" << endl;

                } else {

                        os << ") node[left]{$" << (i * ((BP_HEIGHT - 1) / 5))
                        * (sum_max / 5) << "$} -- (0.5," << (i
                        * ((double) (BP_HEIGHT - 1) / 5)) + 0.5 << ");" << endl;
                }
        ...
        if (count % 3 == 0 || it == itend || count + 1 == 20 ) {
...
template<class type> static void write_ybarPlotHead(fstream& tex,
    ...
    /* quick hack to solve min=0 problem for logarithmic scaling */
...
static void get_RGB_color(fstream& tex, struct AllData& alldata, int usedFunctionGroupMapSize ) {
            
    int R=0,G=0,B=0, colorsteps=20;
        
    //write Heatmap Scale colors
        
        R=255, G=255, B=255;
        for(int i = 1; i <= colorsteps; i++) {
            ...
            tex << "\\definecolor{" << i << "}{RGB}{" << R << "," << G << "," << B << "}" << endl;
        }
 
    
    //write FunctionGroupHistogram Scale colors
        
            
        vector<int>::iterator it_R = alldata.vector_R.begin();
        vector<int>::iterator it_G = alldata.vector_G.begin();
        vector<int>::iterator it_B = alldata.vector_B.begin();
                    
                        
        for(int i = 1; i <= usedFunctionGroupMapSize; i++)
        {
                    
            tex << "\\definecolor{" << i + colorsteps << "}{RGB}{" << *it_R << "," << *it_G << "," << *it_B << "}" << endl;
...
bool CreateTex(AllData& alldata) {
    ...
    write_functionTableExTime(tex_file, alldata);
    write_functionTableInvoc(tex_file, alldata);
