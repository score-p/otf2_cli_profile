...
/*void top_nodes(Data data, const int num){
    // 1. iterate through all nodes find top x nodes and save them
    //    or just sort data and get x nodes
   ...
   std::sort(data.begin(), data.end(), [](Node* a, Node* b){
    ...
    // 2. 
        //option 1
        //  draw these nodes + all ancestors
        //  need to figure out how to tell if one node already drwan
        //  could be used to optimize filter_min_ratio
        //  dot writer class would be usefull
        //      wich should have function print ancestor
        //option 2
        //  check every time if node is a top node or have descendent who are
    for(auto i = 0; i < num; ++i){
        // todo
        // dot_writer.write_node(data[i]);

    }
}*/

/*void draw_node(std::ofstream& result_file, const Node* region, const Params& params, double timerange, double min_time){
   // filter nodes
...
/*void print_dot(const Data& data, const Params& params) {
    ...
    for( const auto& region : data ){
        ...
        // draw_node(result_file, region, params, timerange, min_time);
    }
